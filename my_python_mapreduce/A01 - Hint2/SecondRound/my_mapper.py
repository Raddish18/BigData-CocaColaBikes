...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
	...
	for line in my_input_stream:
		...
		if ((int(info[0]) == 0) and (int(info[5]) == 0) and (info[1] == "Fitzgerald's Park")):

			fmt = "%d-%m-%Y %H:%M:%S"
			#fmt = "%H"
			date_only = datetime.datetime.strptime(info[4], fmt)
				
			df.append(date_only.hour)
			days.append(calendar.day_name[(datetime.datetime.strptime(info[4], fmt)).weekday()])
			

	count = Counter(df)
		
	for item in count:
		mystr = days[0]+"_"+str(item)+"\t"+"("+str(count[item])+")" + "\n"
